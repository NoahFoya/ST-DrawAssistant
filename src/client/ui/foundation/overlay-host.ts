 ...
 * @description 统一浮层与帮助说明气泡宿主管理器 (OverlayHost)
 *
 * 核心设计：
 * 1. 解决 DOM 节点游离挂载到 document.body 导致脱离主题 Token 继承的问题；
 * 2. 将 portal 挂载到主弹窗容器内，确保气泡与弹窗共享相同的 CSS 变量作用域与 z-index 堆叠上下文；
 * 3. 内置碰撞检测与智能上下翻转算法 (Collision Detection & Auto-Flip)，防止气泡被浏览器视口边缘截断；
 * 4. 采用单例互斥设计，在 Tab 切换或弹窗关闭时通过 `dismissAll()` 兜底清理所有浮层，根治残留问题。
 */

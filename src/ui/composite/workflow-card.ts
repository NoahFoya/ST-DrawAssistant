/**
 * ComfyUI 工作流预设管理器 (WorkflowPresetManager)
 *
 * 功能：
 * 1. 提供 ComfyUI 工作流预设方案的快速切换、新增、重命名、另存为与出厂重置；
 * 2. 支持工作流原生 JSON 结构预览，提供语法校验与错误即时反馈；
 * 3. 内置工作流节点变量扫描算法，自动分析提示词、种子、尺寸与模型占位符映射；
 * 4. 提供可视化诊断弹窗，支持占位符匹配报告查看与安全替换；
 * 5. 追踪工作流 JSON 脏状态，联动预设工具栏进行保存提示。
 *
 * Tips：
 * 1. ComfyUI 节点连线数组结构敏感，变量注入与替换严禁污染非目标输入字段；
 * 2. 外部导入工作流必须校验 JSON 结构合法性，防止非法数据导致执行失败。
 */

 ...
 *
 * 功能：
 * 1. 组合预设方案切换下拉框与 8 键图标操作族；
 * 2. 支持新建、保存、另存为、重命名、导入、导出、重置与删除预设；
 * 3. 实时响应表单脏状态，当配置发生变动时高亮保存按钮与提示文案。
 *
 * Tips：
 * 1. 内置预设方案 (isBuiltin) 自动禁用删除按钮，防止核心预设被误删；
 * 2. 导入按钮集成隐藏式文件选择 input，选中 JSON 文件后自动回调解析。
 */

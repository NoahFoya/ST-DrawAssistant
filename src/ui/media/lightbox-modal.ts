 ...
 * @description 全屏图像查看器组件 (LightboxModal)
 *
 * 核心功能：
 * 1. 提供原图级别的全屏高保真图像预览与展示；
 * 2. 支持平滑缩放、视口拖拽平移、适应屏幕与原始比例复位等视窗操作；
 * 3. 支持图片组间的前后翻页浏览，并提供键盘方向键导航与遮罩快捷退出；
 * 4. 提供图像文件下载与生成元数据抽屉联动入口。
 *
 * 注意事项：
 * 1. 拖拽缩放时需限制最大与最小比例边界，防止图像失真或无法找回视口；
 * 2. 弹窗打开时需监听并接管键盘事件，关闭时必须彻底解绑并恢复背景滚动。
 */

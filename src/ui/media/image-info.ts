/**
 * 图像元数据检查器抽屉组件 (ImageInfoModal)
 *
 * 功能：
 * 1. 结构化解析并呈现生图技术元数据 (生图引擎、生成耗时、超参数与随机种子)；
 * 2. 呈现正向/反向提示词与 LoRA 模型列表，支持一键复制与快捷复用；
 * 3. 展示图像缩略图、文件尺寸与存储类型规格；
 * 4. 提供原始响应 JSON 预览与复制能力，辅助调试与参数复现。
 *
 * Tips：
 * 1. 历史图像记录可能缺失部分元数据字段，渲染时提供缺省值防崩溃保护；
 * 2. 抽屉打开时为 Blob 分配 Object URL，关闭或销毁时及时撤销释放。
 */

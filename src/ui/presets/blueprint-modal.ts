...
export function openBlueprintModal(
    ...
    try {
        if (workflowJsonStr.trim()) {
            let rawObj = JSON.parse(workflowJsonStr);
        ...
        FeedbackService.toastError('当前工作流 JSON 存在语法错误，无法启动蓝图可视化编辑器');
        return;
    }
    ...
    const renderInspector = () => {
        ...
        Object.entries(nodeData.inputs || {}).forEach(([key, val]) => {
            ...
            if (Array.isArray(val)) {
                ...
                getMacroVariables().forEach((v) => {
                    opts += `<option value="${v.variable}">${v.variable} (${v.label})</option>`;
                });
    ...
    saveBtn.addEventListener('click', () => {
        try {
            ...
            FeedbackService.toastError(`保存失败: ${err instanceof Error ? err.message : String(err)}`);
        }
    });

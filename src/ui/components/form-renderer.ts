/**
 * 表单配置渲染器 (FormRenderer)
 * 解析表单结构定义 (FormRowSchema / SectionCardSchema)，统一构建布局卡片、
 * 字段标签与具体控件，并建立与 SettingsStore 的响应式双向数据流与生命周期管理。
 */
...
export class FormRenderer<TState extends object> implements IDisposable {
    ...
    public renderCard(schema: SectionCardSchema<TState>): HTMLElement {
        const card = createCard({
            ...
            defaultCollapsed: schema.collapsible && schema.defaultOpen === false
        });
    ...
    /**
     * 根据单行 Schema 渲染标准表单行，创建行容器、标签与对应输入控件并绑定数据
     */
    public renderRow(schema: FormRowSchema<TState>): HTMLElement {
        ...
        const controlId = handleKey ? `da-ctrl-${String(handleKey).replace(/\./g, '-')}` : undefined;

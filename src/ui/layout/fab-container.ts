...
export class FABContainer implements IDisposable {
    ...
    private applyStyles(): void {
        ...
        const opacity = settings.fabOpacity ?? 0.95;
        ...
        if (settings.fabCustomIcon) {
            iconSlot.innerHTML = `<img src="${settings.fabCustomIcon}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;pointer-events:none;" alt="FAB Icon" />`;
        } else {
            iconSlot.innerHTML = getPresetSvg(settings.fabPresetIcon);
        }
    }
    ...
    private clampToViewport(): void {
        ...
        const maxX = window.innerWidth - this._fabElement.offsetWidth;
        const maxY = window.innerHeight - this._fabElement.offsetHeight;

        if (rect.left > maxX || rect.top > maxY || rect.left < 0 || rect.top < 0) {

...
import { createCard, createCardHeader, createRow, createCol, createFieldLabel, bindPresetToolbar } from '../../../ui';
import {
...
/**
 * 角色管理器卡片构建辅助函数 (基于通用卡片与头部组件)
 */
function createCardSection(options: {
    title: string;
    description?: string;
    action?: HTMLElement;
    ...
    if (options.renderBody) {
        options.renderBody(card.body);
    }
    return card.root;
}

/**
 * 角色管理器字段行构建辅助函数 (基于通用行容器与字段标签组件)
 */
function createFieldRow(options: {
    ...
    const isBlock = options.isBlock || options.control.tagName === 'TEXTAREA';
    ...
    if (isBlock) {
        ...
        col.slots[1].appendChild(options.control);
        return col.root;
    }

    const row = createRow(['left', 'right'], { align: 'center', divided: true });
    row.slots[0].appendChild(fieldLabel);
    row.slots[1].appendChild(options.control);
    return row.root;
}

export function createCharacterTabView(storage: CharacterStorage): HTMLElement {
    ...
    subPanes.character = createCharacterPane(storage);
    subPanes.outfit = createOutfitPane(storage);
    subPanes.scheme = createEnableSchemePane(storage);
    subPanes.template = createInjectionTemplatePane(storage);
    subPanes.formula = createRegexFormulaPane(storage);
    ...
    const switchSubTab = (id: string) => {
        ...
        navBar.querySelectorAll('.da-sub-tab-btn').forEach((btn) => {
            ...
            if (b.dataset.subTabId === id) {
                b.className = 'da-btn da-btn--primary da-sub-tab-btn';
            } else {
                b.className = 'da-btn da-btn--secondary da-sub-tab-btn';
            }
        });
    ...
    subTabs.forEach((tab) => {
        ...
        btn.className = `da-btn ${tab.id === activeSubTabId ? 'da-btn--primary' : 'da-btn--secondary'} da-sub-tab-btn`;
...
// ═══════════════════════════════════════════════════════════════════════════════
function createCharacterPane(storage: CharacterStorage): HTMLElement {
...
// ═══════════════════════════════════════════════════════════════════════════════
function createOutfitPane(storage: CharacterStorage): HTMLElement {
    const pane = document.createElement('div');
...
// ═══════════════════════════════════════════════════════════════════════════════
function createEnableSchemePane(storage: CharacterStorage): HTMLElement {
    const pane = document.createElement('div');
...
// ═══════════════════════════════════════════════════════════════════════════════
function createInjectionTemplatePane(storage: CharacterStorage): HTMLElement {
    const pane = document.createElement('div');
...
// ═══════════════════════════════════════════════════════════════════════════════
function createRegexFormulaPane(storage: CharacterStorage): HTMLElement {
    const pane = document.createElement('div');
    ...
    const renderFormulasList = () => {
        ...
        addBtn.className = 'da-btn da-btn--secondary';
    ...
    pane.appendChild(
        createCardSection({
            title: '角色宏后缀公式规则 (Macro Formula Rules)',
            renderBody: (body) => {

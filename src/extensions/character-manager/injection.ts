...
export function resolveInnerOutfits(
    ...
    return renderedList.map(t => cleanUnfilledTemplatePlaceholders(t)).filter(Boolean).join('\n');
}

/**
 * 清洗模板渲染后的多余空行与全空占位符行
 */
export function cleanUnfilledTemplatePlaceholders(text: string): string {
    if (!text) return '';
...
export function renderCharacterAndOutfitInjection(textContent: string): {
    ...
    activeChars.forEach(char => {
        ...
        charRenderedList.push(cleanUnfilledTemplatePlaceholders(itemText));
    });
    ...
    activeOutfits.forEach(outfit => {
        ...
        outfitRenderedList.push(cleanUnfilledTemplatePlaceholders(itemText));
    });
    ...
    try {
        ...
        Object.values(wiEntries).forEach(entry => {
            ...
            if (charRegex.test(raw) || outfitRegex.test(raw)) {
                ...
                entry.content = cleanUnfilledTemplatePlaceholders(updated);
            }
        });
...
export function processCharacterPrompt(promptText: string): string {
    ...
    return cleanUnfilledTemplatePlaceholders(strippedXml);
}

...
import { ImageUrlPool } from '../../../../src/client/core/storage';
...
describe('ImageUrlPool (Object URL 内存管理池)', () => {
    ...
    it('并发调用 acquire 同一图片 ID 时应复用正在加载的 Promise，仅创建一次 URL 且引用计数为 2', async () => {
        const pool = new ImageUrlPool({ releaseDelayMs: 5000 });
        ...
        // 两个并发请求同时发起
        const p1 = pool.acquire('img-concurrent', delayedProvider);
        const p2 = pool.acquire('img-concurrent', delayedProvider);

        // 此时 provider 应该只被触发了 1 次
        expect(delayedProvider).toHaveBeenCalledTimes(1);

        // 模拟异步 Blob 就绪
        resolveBlob!(new Blob(['concurrent-data']));
        ...
        // 验证引用计数为 2：第 1 次释放后不应触发销毁倒计时生效
        pool.release('img-concurrent');
        vi.advanceTimersByTime(5000);
        expect(globalThis.URL.revokeObjectURL).not.toHaveBeenCalled();

        // 第 2 次释放，计数归零，5 秒后执行销毁
        pool.release('img-concurrent');
        vi.advanceTimersByTime(5000);
        expect(globalThis.URL.revokeObjectURL).toHaveBeenCalledWith(url1);
    });
